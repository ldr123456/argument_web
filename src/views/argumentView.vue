      ...
      <div class="argu_box">
            ...
            <div class="grid_box">
              ...
              <el-form-item label="文本内容质量">
                <el-checkbox-group v-model="ruleForm.filter_content_dim">
                  <el-checkbox v-for="(item, index) in qualityOption" :key="index" :label="item" :value="item" />
                </el-checkbox-group>
                <el-slider
        ...
        <div class="box_right">
          ...
          <div class="btns">
            ...
            <el-button size="large" @click="exportFile()" :disabled="deriveBtn">
              导出
            </el-button>
          </div>
        </div>
...
const qualityOption = reactive(['总互动量','转发数','评论数','点赞数','阅读数','收藏数'])
    ...
    .then(async () => {
      ...
      await formEl.validate(async (valid, fields) => {
        if (valid) {
          ...
          // ruleForm.file_path = ruleForm.table
          await executeNotebook(ruleForm)
            .then(async (res) => {
              ...
              // await taskStatus(res.data.task_id)
